...
class wpforum{
	...
	function widget($args){
		...
		foreach($posts as $post){
			...
			echo "<li><a href='".$this->thread_link."$post->parent_id.0'>".$this->output_filter($post->subject)."</a> ".__("by:", "wpforum")." ".$this->profile_link($post->author_id)."<br />".$this->format_date($post->date)."</li>";
		}
		echo "</ul>";
	...
	if($sticky_threads){
		...
		foreach($sticky_threads as $thread){
			...
			$out .= "<tr>
							...
							<td>".$this->get_lastpost($thread->id)."</td>
						</tr>";
			}
/********************************************************************************************************/						
					...
					<tr class='$class'>
						...
						<td valign='top'>
							<table width='100%' cellspacing='0' cellpadding='0' class='wpf-meta-table' >
								...
								if($user->description){
									$out .= "<tr><td class='user_desc'>".apply_filters('comment_text', $this->output_filter($user->description))."</td></tr>";
								}
							$out .= "</table>
						</td>
					</tr>";
	...
	global $user_ID;
		...
		$o = "<table width='100% cellspacing='0' cellpadding='0' style='margin:0; padding:0; border-collapse:collapse:' border='0'>
				<tr>
					<td>$image <strong>".$this->get_postname($post_id)."</strong><br /><strong>on: </strong>".$this->get_postdate($post_id)."</td>";
...
//<a name='$g->id' href='http://mac/smf/index.php?action=collapse;c=1;sa=collapse;#1'>General Category</a>"
		...
		foreach($grs as $g){
			if($this->have_access($g->id)){
				...
				foreach($frs as $f){
					...
					$this->o .= "<td nowrap='nowrap' width='11%' align='left' class='wpf-alt'>".__("Topics: ", "wpforum")."".$this->num_threads($f->id)."<br />".__("Posts: ", "wpforum").$this->num_posts_forum($f->id)."</td>";
					
					$this->o .= "<td  width='28%' >".$this->last_poster_in_forum($f->id)."</td>";
					$this->o .= "</tr>";
				}
			$this->o .= "</table>
		...
		$this->o .= "<table>
					<tr>
						<td><img alt='' align='top' src='$this->skin_url/images/new_some.gif' /> ".__("New posts", "wpforum")." <img alt='' align='top' src='$this->skin_url/images/new_none.gif' /> ".__("No new posts", "wpforum")."</td>
					</tr>
	...
	function get_forum_moderators($forum_id){
		...
		return "<i>".__("Moderators:", "wpforum")." $out</i>";
	}
			...
			foreach($results as $result){
				...
				$starter = $wpdb->get_var("select starter from $this->t_threads where id = $result->parent_id");
					...
					$o .= "<tr>
								...
								<td valign='top'>".round($result->score*$const, 1)."%</td>
